...
class i3d_auth_RGBD(nn.Module):
    def __init__(self, args):
        super(i3d_auth_RGBD, self).__init__()
        self._id_classes = args.id_class_num
        self.feature_mode = args.feature_mode
        self.name = 'i3d_auth'
        self.in_channels = 4

        self.end_points = {}
        self.end_points['Conv3d_1a_7x7_RGBD'] = Unit3D(in_channels=self.in_channels, output_channels=64, kernel_shape=[7, 7, 7],stride=(2, 2, 2), padding=(3, 3, 3), name=self.name + 'Conv3d_1a_7x7')
        ...
        if self.feature_mode == 'linear':
            ...
            self.id_logits = nn.Linear(128*4, self._id_classes)
